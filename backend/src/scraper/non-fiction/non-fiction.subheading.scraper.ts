...
export type NonFictionSubHeading = {
  name: string;
  slug: string;
  url: string;
};

export async function scrapeNonFictionSubHeadings(): Promise<
  NonFictionSubHeading[]
> {
  ...
  try {
    ...
    const items = await page.$$eval(
      'a[data-menu_category="Non-Fiction Books"]',
      (anchors) =>
        anchors
          .map((a) => ({
          ...
          .filter(
            (a) =>
              ...
              !a.href.endsWith('/non-fiction-books'),
          )
          .slice(0, 5), // ✅ MAX 5
    );
